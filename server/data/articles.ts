...
**Venue scoring baseline: plan for a 190-par, not 170.** Over the last three IPL seasons, Chinnaswamy has routinely produced first-innings totals in the 180–210 band, and once the innings crosses 200 the win probability becomes far more sensitive to *powerplay wickets* than to the nominal par score. In practice, teams that lose 2+ wickets inside the first 6 overs see their projected total fall by ~25–30 runs at this ground, because the middle-overs “reset” is harder when every mis-hit carries 10 metres further.
...
**Phase scoring: where the game actually swings.** Chinnaswamy’s powerplay run rate often sits around 9.0–9.5 RPO, but the real inflation is late: death overs frequently push beyond 11.5 RPO because set batters can target both sides of the wicket. If you bat first, your KPI is simple: reach 55+ in the powerplay without burning more than one wicket, then treat overs 16–20 as a 60-run target.

**RCB vs SRH tactical lens.** RCB’s anchor-plus-hitters model (Kohli stabilising, Salt/Patidar finishing) maps well to a venue where “par” is a moving number. SRH’s ceiling is similarly high if Head and Klaasen arrive with wickets in hand; Klaasen in particular tends to add ~2.0 RPO to a chase once he survives his first 10 balls.

**Bottom line.** If you’re batting first, 205 isn’t a flex — it’s insurance. If you’re chasing, your key number is 10.0 RPO after 10 overs: keep it there and the last 10 overs at Chinnaswamy usually supply the rest.`,
  },

  {
    ...
    chartData: [
      {
        label: `Field first (model)`,
        value: 54,
        color: `#10b981`
      },
      {
        label: `Bat first (model)`,
        value: 46,
        color: `#f59e0b`
      }
    ],
    content: `At Chinnaswamy, the toss isn’t destiny — but it’s the closest thing T20 analytics has to a small pre-match edge. The mechanism is simple: once the outfield dampens, boundary conversion rises and bowling execution becomes noisier.
...
**Matchup 1: Kohli’s tempo vs SRH’s variations.** When Kohli starts at a strike rate of 120–130 through 15 balls, RCB’s projected total is fine *if* wickets are intact. The danger is a two-wicket powerplay: that forces Patidar to stabilise and delays the “overload” phase where RCB want 12+ RPO late.
...
**Par score: 175–185, but the distribution is wide.** Recent IPL seasons at Wankhede tend to cluster around 170–190 first-innings totals. That doesn’t mean 180 is safe; it means 180 is *normal*. The reason Wankhede remains more balanced than Bengaluru is that the boundaries are slightly larger and the sea-level air gives bowlers a touch more control over length.
...
**Bottom line.** If you’re batting first, 185 is the number that flips the pressure back onto the chasing team. If you’re chasing, your KPI is to arrive at the 12-over mark needing 9.0 RPO or less — because Wankhede’s last 8 overs are built for acceleration.`,
...

  {
    ...
    chartData: [
      {
        label: `Field first (typical)`,
        value: 52,
        color: `#10b981`
      },
      {
        label: `Bat first (typical)`,
        value: 48,
        color: `#f59e0b`
      }
    ],
    content: `Wankhede doesn’t always have Bengaluru-style dew, but when the humidity rises the second innings can still become a higher-scoring environment. The toss decision should be framed as a risk trade: do you prefer setting a number with a dry ball, or chasing with slightly more outfield value?
...
**Matchup 1: Rohit vs new-ball swing.** Rohit’s powerplay strike rate tends to climb when the bowler misses length by even a fraction; at Wankhede that fraction is punished. If KKR can keep him to under 8.5 RPO personal scoring rate in the first 3 overs, they reduce MI’s 185 probability meaningfully.
...
**Bottom line.** Guwahati doesn’t demand 205. It demands discipline: one collapse over can be unrecoverable because the middle overs don’t always offer the same easy catch-up acceleration as the bigger grounds.`,
...

  {
    ...
    content: `At Barsapara, toss logic tends to be less absolute than at the extreme batting grounds. Instead of “always chase”, captains should ask: will the pitch slow, and do we have the bowling to defend 170?
...
**Probable XIs (projection).** RR’s spine typically includes a left-handed power opener (Jaiswal) and a wrist-spin threat. CSK’s spine includes a deep batting order and a late-overs control option (Jadeja archetype).

**Matchup 1: Jaiswal’s powerplay vs swing.** If CSK can survive the first 12 balls without a boundary burst, they can push the powerplay run rate down into the 8.0 band, which is meaningful at this venue.
...
**Player spotlight: why Bumrah-type value is nonlinear.** A top death bowler isn’t worth “a little more” than average; he changes the distribution of outcomes. Removing one 8-run over at the death and replacing it with a 14-run over swings win probability far more than a small improvement in middle overs.
...
export const analyticsCards: AnalyticsCard[] = [

  {
    ...
    value: "205 is par at Chinnaswamy",
    description: "On small, high-carry venues, one powerplay wicket is worth ~12–15 runs in projected totals; set 205 as the first-innings insurance line.",
  },
  {
    type: "player",
    title: "Player leverage",
    value: "Bumrah-type death overs",
    description: "An elite death bowler can swing ~8–12 runs across 4 overs vs replacement pace, which is often a 5–8 point win-probability shift.",
    playerIds: [2],
  },
  {
    type: "matchup",
    title: "Matchup to watch",
    value: "Klaasen vs left-arm spin",
    description: "If the bowling side can force Klaasen to start against a pace-off left-arm spell, boundary probability drops early and the chase model tightens.",
    playerIds: [9, 18],
  },
];
